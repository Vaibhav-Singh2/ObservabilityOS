...
function generateMockAnalysis(input: IncidentPromptInput): IncidentAnalysis {
  ...
  // Heuristic checking
  const isFundsError = errorMessages.some((msg) => msg.includes("insufficient funds") || msg.includes("err_funds"));
  const isDbError = errorMessages.some((msg) => msg.includes("db") || msg.includes("mongo") || msg.includes("connection pool") || msg.includes("connection failed"));
  const isTimeoutError = errorMessages.some((msg) => msg.includes("timeout") || msg.includes("timed out") || msg.includes("latency"));
