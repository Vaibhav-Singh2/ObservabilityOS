...
export default function SearchView({ project, services, initialLogs }: SearchViewProps) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 focus:border-indigo-500 text-slate-200 placeholder:text-slate-650 rounded-lg py-2.5 pl-10 pr-4 text-sm focus:outline-none transition-colors"
            />
          </div>
          <button
            type="submit"
            disabled={isSearching}
            className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-850 px-5 text-sm font-semibold rounded-lg text-white transition-colors flex items-center gap-2 shrink-0 cursor-pointer"
          >
      ...
      <div className="bg-slate-950 border border-slate-900 rounded-xl overflow-hidden">
        ...
        {logs.length === 0 ? (
          ...
          <div className="divide-y divide-slate-900/60 font-mono text-[11px] text-slate-300">
            {logs.map((log) => {
