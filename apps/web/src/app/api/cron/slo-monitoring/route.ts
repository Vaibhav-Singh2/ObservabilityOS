...
export async function GET(request: Request) {
  try {
    ...
    if (!process.env.CRON_SECRET) {
      console.error("[Cron SLO Monitoring] CRON_SECRET not configured");
    ...
    console.log(
      `[SLO Monitoring] Found ${services.length} services with configured SLOs.`,
    );
    ...
    for (const service of services) {
      ...
      if (!project) {
        console.warn(
          `[SLO Monitoring] Project ${service.projectId} not found for service ${service.name}. Skipping.`,
        );
        continue;
      }
      ...
      for (const slo of slos) {
        ...
        if (currentStatus !== previousStatus) {
          console.log(
            `[SLO Monitoring] Transition detected on service ${service.name} for SLO "${slo.name}": ${previousStatus} -> ${currentStatus}`,
          );
          ...
          const dispatchResult = await dispatchMultiChannelSloAlert(
            project,
            alertPayload,
          );
    ...
    return NextResponse.json(
      {
        error: {
          ...
          message: "Cron SLO monitoring trigger failed",
        },
      },
      { status: 500 },
    );
