...
import { connectToDatabase, Project, Service, Log, User, Incident } from "@repo/db";
...
async function run() {
  ...
  for (const s of stats) {
    ...
    console.log(`- Avg Latency: ${s.avgLatency !== null ? Math.round(s.avgLatency) + "ms" : "N/A"}`);
    console.log(`- Open Incidents: ${isAnomalous ? "Yes (Red)" : "No (Green/Yellow)"}`);
  }
  ...
  console.log(`Search level=error count: ${errorLogsSearch.length} (Expected: 5)`);
  ...
  console.log(`Search message="insufficient" count: ${textSearch.length} (Expected: 5)`);
  ...
  console.log(`Search message="handshake" count: ${baselineSearch.length} (Expected: 1)`);
