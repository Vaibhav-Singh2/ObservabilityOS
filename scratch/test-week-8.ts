...
import { connectToDatabase, Project, Service, Log, Incident, Comment, User } from "@repo/db";
import { generatePostMortemMarkdown, PostMortemIncident, PostMortemLog, PostMortemComment } from "../apps/web/src/lib/postmortem";
...
async function run() {
  ...
  if (!project) {
    console.error("No project found. Run test-settings.ts or test-anomaly.ts first.");
    process.exit(1);
  }
  ...
  if (updatedService?.runbookUrl === testUrl && updatedService?.troubleshootingSteps === testSteps) {
    console.log("✅ Success: Service runbookUrl and troubleshootingSteps updated correctly!");
  } else {
  ...
  await Incident.deleteMany({ projectId: project._id, title: "Test DB Outage Anomaly" });
  ...
  const incident = await Incident.create({
    ...
    summary: "Service verify-service is experiencing a database connectivity degradation.",
    rootCause: "MongoDB primary node replication lag exceeded threshold after a bulk write operation.",
  ...
  const comment1 = await Comment.create({
    ...
    userId: user._id,
    content: "Investigating the primary replication lag. CPU on primary replica node is at 88%.",
  });
  ...
  const commentsList = await Comment.find({ incidentId: incident._id }).sort({ createdAt: 1 });
  if (commentsList.length === 2) {
    console.log("✅ Success: SRE comments successfully added to the incident!");
  } else {
    console.error(`❌ Failed: Expected 2 comments but found ${commentsList.length}`);
    process.exit(1);
  }
  ...
  const postMortemMd = generatePostMortemMarkdown(pmIncident, pmLogs, pmComments);
  ...
  if (
    ...
    console.log("✅ Success: Post-mortem generated with correct metadata and SRE comments!");
  } else {
  ...
  if (commentsAfterDelete.length === 1) {
    ...
    console.error(`❌ Failed: Expected 1 comment after deletion but found ${commentsAfterDelete.length}`);
    process.exit(1);
  }
